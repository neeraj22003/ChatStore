...
class EbuyService {
 ...
 Future <List<dynamic>?> searchItems(String query)async{
  ...
  final String browseurl='https://api.ebay.com/buy/browse/v1/item_summary/search?q=';
  if(token==null)return null;
  final response=await http.get(Uri.parse('$browseurl$query'),
   headers: {
 ...
 Future<Map<String,dynamic>?> itemDiscription(String itemid)async{
  ...
  final response =await http.get(Uri.parse('$browsurl$itemid'),
  headers: {
    'Authorization':"Bearer $token",
